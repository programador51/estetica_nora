...
import { OverviewCalculation, ReturnUseService, StateUseDay } from "./types";
import { useEffect, useState } from "react";

const INITIAL_STATE:StateUseDay = {
  isLoading:false,
  services:[],
  durationOnMinutes:0,
  total:0
...
export default function useDay(id?: number):ReturnUseService {
  ...
  const promptCancelation = async () => {
    const response = await promptConfirmation({
      title: "Borrar asueto",
  text: "¿Estás seguro de que deseas eliminar este asueto?",
    });
  };
